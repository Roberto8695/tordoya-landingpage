...
interface DeleteServicioDialogProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (servicio: ServicioDTO) => void;
  isDeleting?: boolean;
}

export default function DeleteServicioDialog({
  servicio, isOpen, onClose, onConfirm, isDeleting = false,
      ...
      {isOpen && servicio && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <MotionDiv initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }}
            ...
            className="relative w-full max-w-md rounded-2xl border border-primary/10 bg-white p-6 shadow-2xl sm:p-8" role="alertdialog"
            aria-labelledby="delete-servicio-title" aria-describedby="delete-servicio-description">
            ...
            <h2 id="delete-servicio-title" className="mt-4 text-center text-lg font-bold text-foreground">Eliminar servicio</h2>
            <div id="delete-servicio-description" className="mt-2 text-center text-sm text-foreground/60">
              <p>¿Estás seguro de eliminar <strong className="text-foreground">{servicio.nombre}</strong>?</p>
              <p className="mt-2 rounded-xl bg-danger/5 px-4 py-3 text-xs font-medium text-danger">Esta acción no se puede deshacer.</p>
            </div>
            <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <MotionButton whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} onClick={onClose} disabled={isDeleting}
              ...
              <MotionButton whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} onClick={() => onConfirm(servicio)} disabled={isDeleting}
                ...
                {isDeleting ? <><svg className="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" opacity="0.25" /><path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Eliminando...</> : "Eliminar"}
              </MotionButton>
            </div>
          </MotionDiv>
        </div>
