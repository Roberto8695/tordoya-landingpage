...
function stripName(subtitle: string): string {
  const parts = subtitle.split("·").map((s) => s.trim());
  if (parts.length > 1 && /^(Dr\.|Dra\.)/i.test(parts[0]!)) {
...
function mapButtons(style: string): "accent" | "pink" | "ghost" {
  if (style === "primary") return "accent";
  if (style === "secondary") return "pink";
  return "ghost";
}
    ...
    <section id="inicio" className="relative h-screen overflow-hidden text-white">
      <AnimatePresence mode="wait">
        {slides.map((slide, index) => {
          ...
          const showParticlesA = index % 2 === 0;
                ...
                <div className="flex h-full flex-col justify-center lg:relative lg:flex-row lg:items-center lg:justify-between">
                  <div className="z-10 max-w-2xl lg:max-w-xl lg:py-16">
                    <MotionSpan
                      ...
                      className={`inline-flex items-center gap-2 rounded-full ${theme.badgeBg} backdrop-blur-md px-4 py-2 text-xs sm:text-sm font-semibold tracking-[0.18em] uppercase text-white/95 border border-white/15 shadow-lg`}
                    >
                    ...
                    <MotionH1
                      ...
                      animate={isActive ? { opacity: 1, y: 0 } : {}}
                      transition={{ duration: 0.5, delay: slideName ? 0.3 : 0.2 }}
                      className={`mt-2 font-bold leading-tight tracking-tight text-balance ${
                        slideName ? "text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-accent" : "text-3xl sm:text-4xl md:text-5xl lg:text-6xl"
                      }`}
                    >
                      {slide.title}
                    </MotionH1>
                    ...
                    >
                      <a
                        ...
                        className={`inline-flex items-center justify-center gap-2 rounded-full px-7 py-3.5 text-sm font-bold shadow-xl transition-all duration-300 hover:-translate-y-0.5 hover:shadow-2xl ${
                          mapButtons(slide.cta1.style) === "accent"
                            ? "bg-accent text-primary shadow-accent/40"
                            : mapButtons(slide.cta1.style) === "pink"
                    ...
                    <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
                      <MotionDiv
                        ...
                        className={`h-[55vh] w-[55vh] rounded-full blur-3xl -translate-y-[10%] ${
                          showParticlesA ? "bg-[#00b7fa]/20" : "bg-[#e85d9a]/20"
                        }`}
                      />
                    </div>

                    <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none">
                      <div className="h-[58vh] w-[58vh] rounded-full border border-white/10 -translate-y-[8%] bg-white/[0.04]" />
                    </div>

                    <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none">
                      <div className="h-[32vh] w-[32vh] rounded-full border border-white/10 -translate-y-[5%] bg-white/[0.06]" />
                    </div>
